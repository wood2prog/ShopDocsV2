...
- The room/job form is **schema-driven**: `questions.json` (shipped next to the exe, copied via `CopyToOutputDirectory`) defines sections → questions → (for `list`-type questions) item fields. `QuestionDef.CatalogSource` points a dropdown at a catalog table (`materials`/`finishes`/`countertops`/`pulls`/`hardware_colors`/`accessories`); `CatalogFilterBy` narrows it by a sibling field's value (only `countertops.color` filtered by `material` today). An `accessories` answer stores the accessory's `Label` ("Name – Model#"), and `catalogLink: true` adds a read-only Product Page column (after the item's fields) showing that label as a link to the accessory's catalog URL, looked up at display/click time (like `colorPreview` adds the Swatch column). Changing question behavior usually means editing `questions.json`, not C# — the UI (`RoomFormBuilder`) interprets the schema generically.
...
- Keyboard shortcuts live on the menu items themselves (`ToolStripMenuItem.ShortcutKeys`). `MainForm` lists the rebindable ones as `KeyBindingCommand`s with stable ids (`room.add`, `group.<questionId>`, ...); Tools > Keyboard Shortcuts (`KeyBindingsDialog`) edits them, and only overrides of the defaults are saved, under `KeyBindings` in `settings.json` (`KeyBindingSettingsStore`). Room > Go to Group has one item per List question, whose default key comes from that question's `shortcut` in `questions.json`; it scrolls to the list and starts a new line (reusing a blank last line). `settings.json` is shared by several owners, so write it through `Files/SettingsFile.Update`, which keeps the other properties.
